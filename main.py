...
# app/main.py
#
# FastAPI Application Factory — Pacha (Green) Cover
#
# Responsibilities:
...
def create_app() -> FastAPI:
    ...
    # ── CORS ───────────────────────────────────────────────────────────────
    # Allows the Flutter/React frontend to call the API from the browser.
    app.add_middleware(
        CORSMiddleware,
    ...
    async def add_security_headers(request: Request, call_next):
        """
        Adds basic security headers to protect Web and Flutter Web clients.
    ...
    async def root():
        return {
            "message": "🌿 Pacha Cover API is running. Visit /docs for the API reference.",
            "docs": "/docs",
        }
